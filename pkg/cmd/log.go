...
func (o *LogOptions) Run() error {
	...
	if o.LabelSelector != "" {
		...
		o.Name = list.Items[0].GetName()
	}

	apiPath := fmt.Sprintf("%s/%s/namespaces/%s/%s/%s/log", apiPrefix, o.GroupVersion, *o.kubeFlags.Namespace, o.Resource, o.Name)
	if o.ContainerName != "" {
	...
	fmt.Println(string(kubearchiveLog))
	return nil
}
