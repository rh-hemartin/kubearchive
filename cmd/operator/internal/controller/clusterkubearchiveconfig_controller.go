...
func (r *ClusterKubeArchiveConfigReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if ckaconfig.DeletionTimestamp.IsZero() {
		...
		if controllerutil.ContainsFinalizer(ckaconfig, resourceFinalizerName) {
			...
			if _, err := reconcileAllCommonResources(ctx, r.Client, r.Mapper, constants.SinkFilterGlobalNamespace, nil, r.MonitorAllNamespaces); err != nil {
				return ctrl.Result{}, err
			}
	...
	if _, err := reconcileAllCommonResources(ctx, r.Client, r.Mapper, constants.SinkFilterGlobalNamespace, ckaconfig.Spec.Resources, r.MonitorAllNamespaces); err != nil {
		return ctrl.Result{}, err
	}
